<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:ora="http://www.oracle.com/XSL/Transform/java">
        ...
        <xsl:choose>
          ...
          <xsl:when test="$special-type='1'">
            <xsl:element name="text">
              <xsl:text>select sid,sparentid</xsl:text>
              <xsl:for-each select="*">
                <xsl:value-of select="','"/>
  ...
  <xsl:template name="text-content-bef">
    ...
    <xsl:choose>
      <xsl:when test="self::query[name(..)='from']">
        <xsl:if test="@join">
          <xsl:value-of select="' '"/>
      ...
      <xsl:when test="self::column and not(name(..)='insert')">
        <xsl:if test="@group and not(@group=1) and  ancestor::select">
    ...
    <xsl:if test="self::call[name(..)='query']">
  ...
  <xsl:template name="text-content-aft">
    ...
    <xsl:if test="self::query[not(call) and name(..)='from' ]">
      <xsl:if test="@order">
        <xsl:text> order by </xsl:text>
        <xsl:value-of select="@order"/>
      </xsl:if>
    ...
    <xsl:if test="self::*[@as and name(..)='select']">
      <xsl:text> as</xsl:text>
      <xsl:value-of select="' '"/>
      <xsl:choose>
        <xsl:when test="name(../../..)='root' and not(../../insert) and @title">
          <xsl:text>"</xsl:text>
    ...
    <xsl:if test="self::*[name(..)='select' or name(..)='group']">
      <xsl:if test="not(position()=last())">
        <xsl:text>,</xsl:text>
      </xsl:if>
    </xsl:if>
